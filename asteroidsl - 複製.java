...
public class asteroidsl extends asteroids {
	...
	public String toDataString() {
        ...
        // 假设你有 x[]、y[] 存顶点，以及 asteroidsflag 表示大小
        for (int i = 0; i < x.length; i++) {
            sb.append((int)x[i]).append(",").append((int)y[i]);
            if (i < x.length - 1) sb.append(";");
        }
        sb.append(";").append(asteroidsflag);
        return sb.toString();
    }

    /**
     * 返回一个唯一 ID，用于网络同步时标识这颗小行星
     */
    public String getId() {
        // 这里用 hashCode 生成一个简单的 ID
